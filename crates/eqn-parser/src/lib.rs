...
//!
//! 1. [`Grammar::parse`] reads source text into an untyped [`Ast`], driven by
//!    the [`Grammar`]: the table of operator symbols with their fixity,
//!    precedence and associativity. This crate declares no operators of its
//!    own; `+` is an operator only where a grammar says so, and `\oplus` is
...
//!
//! [`FromAst::parse`] runs both stages. Expression types expose it through
//! [`FromStr`], so `"x + 2 * y".parse::<RingExpr<R>>()` is the intended entry
//! point.
//!
//! # Syntax
//!
//! Always available: numeric literals (`3`, `2.5`), identifiers (`x`, `x_1`,
//! `\theta`; a letter or `_` followed by letters, digits or `_`), calls
...
//! identifier or a number, so `\wedge`, `**` and `\oplus` all work; the lexer
